			...
			<nav class="navbar navbar-default" role="navigation">
				<div class="navbar-header">
					...
					<c:if test="${requestScope.CONFIGS['displaySearchBox'] == true}">
						<button type="button"
							class="navbar-toggle searchButton no-desktop">
							<i class="fa fa-search"></i>
						</button>

						<!-- search bar collapsed -->
						<div id="responsiveSearchFieldGroup" class="no-desktop">
							<input type="text" class="typeahead navbar-toggle" placeholder=""
				...
				<div id="mainNavigation" class="collapse navbar-collapse">
					...
					<ul class="nav navbar-nav navbar-right" id="topMain">
						...
						<c:forEach items="${requestScope.TOP_CATEGORIES}" var="category">
							<li
								class="<sm:activeLink linkCode="${category.description.friendlyUrl}" activeReturnCode="active"/> 
								<c:if test="${fn:length(category.children)>0}">dropdown mega-menu</c:if>">
								<a
								href="<c:url value="/shop/category/${category.description.friendlyUrl}.html"/><sm:breadcrumbParam categoryId="${category.id}"/>"
								class="<c:if test="${fn:length(category.children)>0}">dropdown-toggle</c:if> active <c:if test="${category.code==code}">currentSelectedLink</c:if>"
											...
											<div class="row">
												...
												<c:if test="${requestScope.CONTENT[category.code]!=null}">
													<div class="col-sm-4 col-md-6">
														<c:if
															test="${requestScope.CONTENT[category.code].description!=null}">
															<c:out
																value="${requestScope.CONTENT[category.code].description}"
																escapeXml="false" />
														</c:if>
													</div>
												</c:if>
												...
												<div
													class="mega-menu-items <c:choose><c:when test="${requestScope.CONTENT[category.code]!=null}">col-sm-8 col-md-6</c:when><c:otherwise>col-sm-12 col-md-12</c:otherwise></c:choose>">
													<h4>
														<a
															href="<c:url value="/shop/category/${category.description.friendlyUrl}.html"/><sm:breadcrumbParam categoryId="${child.id}"/>"><c:out
																value="${category.description.name}" /></a>
													</h4>
